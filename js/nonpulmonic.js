const audioBaseUrls = [
  "https://www.coelang.tufs.ac.jp/ipa/consonant_nonpulmonic/sounds/",
  "https://www.coelang.tufs.ac.jp/ipa/sounds/"
];
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  let currentSoundSources = [];
  let currentSourceIndex = 0;
  let pendingSuccessMessage = "";
  let pendingFailureMessage = "";

  function buildAudioSources(soundID) {
    currentSoundSources = audioBaseUrls.map((baseUrl) => `${baseUrl}${soundID}.mp3`);
  ...
  function tryPlayCurrentSource() {
    if (currentSourceIndex >= currentSoundSources.length) {
      handlePlaybackFailure();
      return;
    }

    audioPlayer.src = currentSoundSources[currentSourceIndex];
  ...
  audioPlayer.addEventListener("error", () => {
    if (currentSourceIndex + 1 < currentSoundSources.length) {
  ...
  function playSound(soundID, { successMessage = "", failureMessage } = {}) {
    ...
    pendingFailureMessage =
      failureMessage ||
      `⚠️ 音声の読み込みに失敗しました。通信環境をご確認ください。（記号: ${soundToSymbol[soundID] || soundID}）`;

    audioPlayer.pause();
    audioPlayer.removeAttribute("src");
    audioPlayer.load();

    buildAudioSources(soundID);
    tryPlayCurrentSource();
  }
  ...
  function replaySound() {
    ...
    playSound(correctSound, {
      ...
      failureMessage: "⚠️ 音声を再生できませんでした。通信環境をご確認ください。"
    });
  }
  ...
  function playRandomSound() {
    ...
    playSound(correctSound, {
      ...
      failureMessage: "⚠️ 音声の読み込みに失敗しました。通信環境をご確認ください。"
    });
  }
  ...
  function setupClickListeners() {
    document.querySelectorAll(".clickable").forEach((cell) => {
      cell.addEventListener("click", () => {
        ...
        playSound(selected, {
          failureMessage: `⚠️ 音声の読み込みに失敗しました。通信環境をご確認ください。（記号: ${soundToSymbol[selected] || selected}）`
        });
